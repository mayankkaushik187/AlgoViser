...
int sum;
int n;
int helper(int index,int tot){
	if(index == n - 1){
		return abs(abs(sum - tot) - tot);
	}
	int one = helper(index + 1, tot + a[index]);
	int two = helper(index + 1, tot);
	return min(one,two);
...
void solve() {
	...
	int ans = helper(0,0);//find the min diff between the sum of two sets from 0....n - 1
