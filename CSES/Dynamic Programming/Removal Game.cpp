...
int helper(vector<vector<int>>&dp, vector<int> &arr, int n, int start, int end) {
	if (start >= end) {
	...
	return dp[start][end] = max(arr[start] + min(helper(dp, arr, n, start + 2, end), helper(dp, arr, n, start + 1, end - 1)), arr[end] + min(helper(dp, arr, n, start + 1, end - 1), helper(dp, arr, n, start, end - 2)));
}
