...
int dp(int index,int taken,int minNotTaken){
	if(taken > k)return 0;
	if(index >= n){
		return taken == k;
	}
	int &ans = pd[index][taken][minNotTaken];
	if(ans != -1)return ans;
	int op1 = dp(index + 1,taken, min(minNotTaken,P[index].ss));
	if(P[index].ss < minNotTaken) op1 = mod_add(op1,dp(index + 1,taken + 1,minNotTaken),MOD1);
