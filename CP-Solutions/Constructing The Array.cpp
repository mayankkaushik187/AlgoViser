...
void solve() {
	...
	while (sz(st)) {
		auto front = *st.begin();
		st.erase(st.begin());
		int len = front.ss - front.ff + 1;
		if (len & 1) {
			int mid = (front.ff + front.ss) / 2;
			res[mid] = id++;
			if (mid - front.ff >= 1)st.insert({front.ff, mid - 1});
			if (front.ss - mid >= 1)st.insert({mid + 1, front.ss});
		} else {
			int mid = (front.ff + front.ss - 1) / 2;
			res[mid] = id++;
			if (mid - front.ff >= 1)st.insert({front.ff, mid - 1});
			if (front.ss - mid >= 1)st.insert({mid + 1, front.ss});
		}
