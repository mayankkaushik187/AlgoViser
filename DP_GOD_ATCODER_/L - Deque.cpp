...
int helper(int start,int end){
	...
	return memo[start][end] = max(arr[end] + min(helper(start,end - 2), helper(start + 1, end - 1)),arr[start] + min(helper(start + 1,end - 1), helper(start + 2,end)));
}
...
void solve() {
	...
	int start = 0, end = n - 1;
	int ans = helper(start,end);
	cout << ans - (sum - ans) << nline;
}
