...
/*--------------------------------------------------------------------------------------------------------------------------*/
const int N = 22;
int arr[N][N], memo[N][(1 << N)];
int n;

int dp(int male, int mask) {
	if (male == n) {
		return 1;
	}
	if (memo[male][mask] != -1)return memo[male][mask];
	int ans = 0;
	for (int womenBit = 0 ; womenBit < n ; womenBit++) {
		int setBit = (1 << womenBit) & mask;
		int compatible = arr[male][womenBit];
		if (setBit and compatible) {
			ans += dp(male + 1, mask ^ (1 << womenBit));
			ans %= MOD;
		}
	}
	return memo[male][mask] = ans;
}
