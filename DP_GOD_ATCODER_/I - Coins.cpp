...
void solve() {
	...
	dp[0][0] = 1.00;
	for(int i = 1 ; i <= n ; i++){
		long double pi = arr[i - 1];
		for(int j = 0 ; j <= i ; j++){
			if(j == 0){
				dp[i][j] = (1.00 - pi) * dp[i - 1][j];
			}else if(i != j){
				dp[i][j] = (1.00 - pi) * dp[i - 1][j] + pi * dp[i - 1][j - 1];
			}else{
				dp[i][j] = pi * dp[i - 1][j - 1];
			}
		}
	}
...
long double helper(int i,int head,int tail){
	if(i == n)return 1.00;
	if(memo[i][head] != -1.00)return memo[i][head];
	long double ans = 0.00;
	if(head){
		ans += helper(i + 1, head - 1, tail) * arr[i];
	}
	if(tail){
		ans += helper(i + 1, head, tail - 1) * (1.00 - arr[i]);
	}
	return memo[i][head] = ans;
}
