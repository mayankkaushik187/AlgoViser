...
/*--------------------------------------------------------------------------------------------------------------------------*/
const int N = 1e4 + 5;
const int sumDigs = 100;
int dp[N][2][sumDigs];
string s;
int d;
int helper(int pos,int fl,int sum){
	if(pos >= s.size()){
		return sum == 0;
	}
	int &ans = dp[pos][fl][sum];
	if(ans != -1)return ans;
	int LMT = fl ? (s[pos] - '0') : 9;
	ans = 0;
	for(int i = 0 ; i <= LMT ; i++){
		ans += helper(pos + 1,(i == LMT) and fl, (sum + i) % d);
		ans %= MOD;
	}
	return ans;
}
void solve() {
	...
	cin >> s >> d;
	int ans = (helper(0,1,0) - 1 + MOD) % MOD;
	cout << ans << nline;
}
